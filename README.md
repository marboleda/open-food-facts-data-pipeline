# Open Food Facts (OFF) Data Pipeline
This is a data pipeline for processing data from [Open Food Facts](https://world.openfoodfacts.org/), a crowdsourced food products database that contains info about food products around the world so people can make better decisions about their food purchases.

## Overview
Open Food Facts provides a database dump daily, as detailed [here](world.openfoodfacts.org/data). This allows us to analyse up-to-date food data and gain insights into food & nutrition trends around the world.

## Use Cases

## Architecture
| Technology | Purpose |
| --- | --- |
| Apache Airflow | Workflow Orchestration

## Dashboard

## How-To
### Prerequisites
- Docker Desktop **or** Docker Engine + Docker Compose is [installed](https://docs.docker.com/compose/install)

## Notes and Potential Improvements